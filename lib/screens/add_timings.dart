...
import 'package:app/screens/add_timing.dart';
import 'package:app/screens/timing.dart';
import 'package:cloud_firestore/cloud_firestore.dart';
import 'package:flutter/material.dart';

class AddTimings extends StatefulWidget {
  const AddTimings({super.key, required this.id});
    ...
    return Scaffold(
                ...
                return ListTile(
                  onTap: () {
                    Navigator.of(context).push(
                      MaterialPageRoute(
                        builder: (context) => ViewTiming(timing: timings[index].data(), trip: trip, timingId: timings[index].id, tripId: widget.id,),
                      )
                    ).then((value) => getData());
                  },
                  title: Text(
      ...
      floatingActionButton: FloatingActionButton(
        onPressed: () {
          Navigator.of(context).push(
            MaterialPageRoute(
              builder: (context) => AddTiming(tripId: trip.id, trip: trip,),
            )
          ).then((value) => {
            getData()
          });
